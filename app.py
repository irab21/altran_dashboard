...
if password== 'britishtelecom@2020':
	...
	st.markdown('### Number Of Positive Conversions and Negative Conversions')
	st.write('\n\n')
	st.write('\n\n As can be seen, out of %s total selections:\n\n Only %s Candidates were Offered \n\n Out of the Offered Candidates %s Candidates were Positively Converted \n\n %s Candidates were not Converted'%(total_selections1.values[0],offer_count.values[1],total_selections.values[2,2],total_selections.values[0,0]))
	if st.sidebar.checkbox('Visual',True, key=4):
		fig1=px.pie(status_count, values='Count',names='Status')
		st.plotly_chart(fig1)
	...
	fig10 = make_subplots(rows=1, cols=3, specs=[[{'type':'domain'}, {'type':'domain'},{'type':'domain'}]])
	fig10.add_trace(go.Pie(labels=labels, values=[53,133], name="Selected Candidates Diversity Percentage"),1, 1)
	fig10.add_trace(go.Pie(labels=labels, values=[38,105], name="Offered Candidates Diversity Percentage"),1, 2)
	fig10.add_trace(go.Pie(labels=labels, values=[12,36],name="Joined Candidates Diversity Percentage"),1, 3)
